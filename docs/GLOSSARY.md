...
- **Shared memory domain** — the tool-independent `mnemosyne/memory/` package that owns scopes, records, normalization, paths, storage, retrieval, revision mutable-field and content policy, lifecycle policy, atomic replacement, replacement uncertainty, and shared errors.
