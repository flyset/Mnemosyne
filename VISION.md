...
configurable protocol-header strictness, and MyMCP `0.10.0` adds schema-7
bounded startup-configured session lifetimes; default strictness and fixed
lifetimes remain unchanged unless explicitly selected. Governance remains deferred.
...
marker is independently `0.10.0`.
...
reusable host services follow in that dependency order. Registered sessions bind
the normalized principal, runtime generation, and negotiated MCP context without
becoming credentials, policy decisions, approval, or plugin-visible state.
